...
import {Book09, Book09Search, Book10, ColsTable} from '../../model/so-thu-ly.model';
...
@Component({
  selector: 'app-mau-so-chin',
  templateUrl: './mau-so-chin.component.html',
  providers: [
    DatePipe
  ]
})
export class MauSoChinComponent implements OnInit {
  totalPages: number;
  totalRecords: number;
  ...
  ngOnInit(): void {
    ...
    const listDecisionInBook09: string[] = ['0702', '5502', '0204', '0507', '3906', '5309', '8908', '0704', '5501', '0506', '5304'];
    const listDecisionInStorage: Decision[] = JSON.parse(localStorage.getItem('quyet_dinh'));
    this.listDecision = listDecisionInStorage.filter((decision: Decision) => listDecisionInBook09.includes(decision.deciId));
    this.listGroupLawCode = JSON.parse(localStorage.getItem('bo_luat'));
    this.searchModel.unitId = this.listSpp[0].sppId;
    this.colsTable = [
      {
        title: 'STT/ Ngày, tháng, năm vào sổ thụ lý (1)',
        width: '5%'
      },
      {
        title: 'Vụ án/ Họ và tên bị can (2)',
        width: '7%'
      },
      {
        title: 'QĐ khởi tố vụ án (Số; ngày, tháng, năm; Cơ quan ra QĐ) (3)',
        width: '10%'
      },
      {
        title: 'QĐ khởi tố bị can (Số; ngày, tháng, năm; Cơ quan ra QĐ) (4)',
        width: '12%'
      },
      {
        title: 'Tội danh (Điều luật) (5)',
        width: '13%'
      },
      {
        title: 'Kiểm sát viên, Điều tra viên, Thẩm phán thụ lý (6)',
        width: '10%'
      },
      {
        title: 'QĐ đình chỉ (Số; ngày, tháng, năm; Cơ quan ra QĐ) (7)',
        width: '10%'
      },
      {
        title: 'Lý do đình chỉ (Căn cứ, điều luật áp dụng) (8)',
        width: '10%'
      },
      {
        title: 'QĐ xử lý vật chứng (9)',
        width: '10%'
      },
      {
        title: 'Tác động của Viện kiểm sát (Số; ngày, tháng, năm; Nội dung) (10)',
        width: '10%'
      },
      {
        title: 'Kết luận kiểm sát về “Vụ án/Bị can” đã đình chỉ và việc xử lý vật chứng (11)',
        width: '10%'
      },
      {
        title: 'QĐ hủy bỏ QĐ ĐC (Số; ngày, tháng, năm; Cơ quan ban hành) (12)',
        width: '10%'
      },
      {
        title: 'Quyết định phục hồi (Số; ngày, tháng, năm; Cơ quan ban hành) (13)',
        width: '10%'
      },
      {
        title: 'Ghi chú (14)',
  ...
  getListData() {
    ...
    this.constantService.postRequest(this.constantService.REPORT_URI + 'book09/requestReport/'
      , {
  ...
  exportPdf(): void {
    ...
    this.inProgress = true;
    this.constantService.postRequest(this.constantService.REPORT_URI + 'book09/requestReportPdf/'
      , {
        'fromDate': this.datePipe.transform(this.searchModel.fromDate, 'dd/MM/yyyy'),
        'toDate': this.datePipe.transform(this.searchModel.toDate, 'dd/MM/yyyy'),
        'caseCode': this.searchModel.caseCode,
      ...
      .then(resJson => {
        if (resJson.responseCode === '0000') {
          ...
          const fileName = 'Số_09_sổ_quản_lý_án_hình_sự_đình_chỉ_' +
  ...
  exportExcel(): void {
    ...
    this.inProgress = true;
    this.constantService.postRequest(this.constantService.REPORT_URI + 'book09/requestReportExcel/'
      , {
        'fromDate': this.datePipe.transform(this.searchModel.fromDate, 'dd/MM/yyyy'),
        'toDate': this.datePipe.transform(this.searchModel.toDate, 'dd/MM/yyyy'),
        'caseCode': this.searchModel.caseCode,
  ...
  exportDocx(): void {
    ...
    this.inProgress = true;
    this.constantService.postRequest(this.constantService.REPORT_URI + 'book09/requestReportDocx/'
      , {
        'fromDate': this.datePipe.transform(this.searchModel.fromDate, 'dd/MM/yyyy'),
        'toDate': this.datePipe.transform(this.searchModel.toDate, 'dd/MM/yyyy'),
        'caseCode': this.searchModel.caseCode,
  ...
  search(searchKeyword: string): void {
    const filteredList: Book09[] = [];
    this.copyData().forEach((value: Book09) => {
  ...
  stopEdit(item: Book09): void {
    ...
    this.inProgress = true;
    this.constantService.postRequest(this.constantService.REPORT_URI + 'book09/createOrUpdateNote/'
      , {
