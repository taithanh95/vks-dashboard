...
import {Book33, Book33Search, ColsTable} from '../../model/so-thu-ly.model';
...
@Component({
  selector: 'app-mau-so-ba-ba',
  templateUrl: './mau-so-ba-ba.component.html',
  providers: [DatePipe]
})
export class MauSoBaBaComponent implements OnInit {
  totalPages: number;
  totalRecords: number;
  ...
  ngOnInit(): void {
    ...
    this.colsTable = [
      ...
      {
        title: 'Cơ quan yêu cầu trong nước (3)',
        width: '10%'
      },
      {
        title: 'Số văn bản (4)',
      ...
      {
        title: 'Nội dung yêu cầu TTTP (8)',
        width: '10%'
      },
      {
        title: 'Số văn bản chuyển (9)',
        width: '10%'
      },
      {
        title: 'Nơi nhận thực hiện (10)',
        width: '7%'
      },
      {
  ...
  public checkValidate() {
    ...
    //   this.toastrService.warning('Yêu cầu nhập đúng định dạng ngày, tháng, năm dd/mm/yyyy');
    //   return false;
    // }
  ...
  getListData() {
    ...
    this.constantService.postRequest(this.constantService.REPORT_URI + 'book33/requestReport/'
      , {
  ...
  exportPdf(): void {
    ...
    this.inProgress = true;
    this.constantService.postRequest(this.constantService.REPORT_URI + 'book33/requestReportPdf/'
      , {
        'fromDate': this.datePipe.transform(this.searchModel.fromDate, 'dd/MM/yyyy'),
        'toDate': this.datePipe.transform(this.searchModel.toDate, 'dd/MM/yyyy'),
        'unitId': this.searchModel.unitId
      ...
      .then(resJson => {
        if (resJson.responseCode === '0000') {
          ...
          const fileName = 'SỔ_33_SỔ_THỤ_LÝ_TƯƠNG_TRỢ_TƯ_PHÁP_VỀ_HÌNH_SỰ_CỦA_VIỆT_NAM_' +
