...
import {Book26, Book27, Book27Search, ColsTable} from '../../model/so-thu-ly.model';
...
@Component({
  selector: 'app-mau-so-hai-bay',
  templateUrl: './mau-so-hai-bay.component.html',
  providers: [
    DatePipe
  ]
})
export class MauSoHaiBayComponent implements OnInit {
  totalPages: number;
  totalRecords: number;
  ...
  ngOnInit(): void {
    ...
    const decisionsInBook27: string[] = ['8724'];
    if (decisions.length > 0) {
      this.listDecision = decisions.filter((decision: Decision) => decisionsInBook27.includes(decision.deciId));
    }
    this.colsTable = [
      {
        title: 'STT (1)',
      ...
      {
        title: 'Tên, địa chỉ của pháp nhân (2)',
        width: '8%'
      },
      {
        title: 'Bản án, quyết định có hiệu lực pháp luật (Số; ngày, tháng; Tòa án đã xét xử; Tội danh) (3)',
        width: '20%'
      },
      {
        title: 'Hình phạt, mức án, Biện pháp tư pháp phải thi hành (4)',
        width: '20%'
      },
      {
        title: 'Quyết định thi hành án (5)',
        width: '10%'
      },
      {
        title: 'Quyết định cưỡng chế thi hành án; Lý do (6)',
        width: '9%'
      },
      {
        title: 'Giấy chứng nhận đã chấp hành xong (7)',
        width: '8%'
      },
      {
        title: 'Quyết định thi hành (8)',
        width: '8%'
      },
      {
        title: 'Quyết định cưỡng chế thi hành; Lý do (9)',
        width: '8%'
      },
      {
        title: 'Giấy chứng nhận đã chấp hành xong (10)',
        width: '8%'
      },
      {
        title: 'Ghi chú (11)',
  ...
  getListData() {
    ...
    this.constantService.postRequest(this.constantService.REPORT_URI + 'book27/requestReport/'
      , {
        ...
        'legalCode': this.searchModel.legalCode,
        'legalName': this.searchModel.legalName,
  ...
  exportPdf(): void {
    ...
    this.inProgress = true;
    this.constantService.postRequest(this.constantService.REPORT_URI + 'book27/requestReportPdf/'
      , {
        'fromDate': this.datePipe.transform(this.searchModel.fromDate, 'dd/MM/yyyy'),
        'toDate': this.datePipe.transform(this.searchModel.toDate, 'dd/MM/yyyy'),
        'legalCode': this.searchModel.legalCode,
      ...
      .then(resJson => {
        if (resJson.responseCode === '0000') {
          ...
          const fileName = 'Sổ_27_sổ_kiểm_sát_việc_thi_hành_án_đối_với_pháp_nhân_thương_mại_' +
  ...
  exportExcel(): void {
    ...
    this.inProgress = true;
    this.constantService.postRequest(this.constantService.REPORT_URI + 'book27/requestReportExcel/'
      , {
        'fromDate': this.datePipe.transform(this.searchModel.fromDate, 'dd/MM/yyyy'),
        'toDate': this.datePipe.transform(this.searchModel.toDate, 'dd/MM/yyyy'),
        'legalCode': this.searchModel.legalCode,
  ...
  exportDocx(): void {
    ...
    this.inProgress = true;
    this.constantService.postRequest(this.constantService.REPORT_URI + 'book27/requestReportDocx/'
      , {
        'fromDate': this.datePipe.transform(this.searchModel.fromDate, 'dd/MM/yyyy'),
        'toDate': this.datePipe.transform(this.searchModel.toDate, 'dd/MM/yyyy'),
        'legalCode': this.searchModel.legalCode,
  ...
  search(searchKeyword: string): void {
    const filteredList: Book27[] = [];
    let stt = 1;
    this.copyData().forEach((value: Book27) => {
