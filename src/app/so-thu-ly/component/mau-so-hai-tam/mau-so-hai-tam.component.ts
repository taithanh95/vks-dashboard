...
import {Book27, Book28, Book28Search, ColsTable} from '../../model/so-thu-ly.model';
...
@Component({
  selector: 'app-mau-so-hai-tam',
  templateUrl: './mau-so-hai-tam.component.html',
  providers: [
    DatePipe
  ]
})
export class MauSoHaiTamComponent implements OnInit {
  totalPages: number;
  totalRecords: number;
  ...
  ngOnInit(): void {
    ...
    const decisionsInBook28: string[] = ['8702','5131'];
    if (decisions.length > 0) {
      this.listDecision = decisions.filter((decision: Decision) => decisionsInBook28.includes(decision.deciId));
    }
    this.colsTable = [
      {
        title: 'STT (1)',
      ...
      {
        title: 'Bản án (Số; ngày, tháng, năm; Toà án đã xét xử; Tội danh) (3)',
        width: '20%'
      },
      {
        title: 'Hình phạt, thời hạn phải thi hành (4)',
        width: '20%'
      },
      {
        title: 'Ngày T.H.A (5)',
      ...
      {
        title: 'Cơ quan, tổ chức được giao giám sát, giáo dục (6)',
        width: '10%'
      },
      {
        title: 'Vi phạm nghĩa vụ (Ngày, tháng, năm; Nội dung vi phạm) (7)',
        width: '8%'
      },
      {
        title: 'Quyết định xử lý vi phạm (Số; ngày, tháng, năm; Hình thức xử lý) (8)',
        width: '8%'
      },
      {
        title: 'Chết, (Ngày, tháng, năm) (9)',
        width: '8%'
      },
      {
        title: 'Giấy chứng nhận chấp hành xong hình phạt (Số; ngày, tháng, năm) (10)',
        width: '8%'
      },
      {
        title: 'Ghi chú (11)',
  ...
  getListData() {
    ...
    this.constantService.postRequest(this.constantService.REPORT_URI + 'book28/requestReport/'
      , {
  ...
  exportPdf(): void {
    ...
    this.inProgress = true;
    this.constantService.postRequest(this.constantService.REPORT_URI + 'book28/requestReportPdf/'
      , {
        'fromDate': this.datePipe.transform(this.searchModel.fromDate, 'dd/MM/yyyy'),
        'toDate': this.datePipe.transform(this.searchModel.toDate, 'dd/MM/yyyy'),
        'accuCode': this.searchModel.accuCode,
      ...
      .then(resJson => {
        if (resJson.responseCode === '0000') {
          ...
          const fileName = 'Sổ_28_kiểm_sát_thi_hành_án_hình_phạt_bổ_sung_' + this.dateService.convertDateToStringByPattern(Date.now(), 'yyyyMMdd_HHmmss') + '.pdf';
  ...
  exportExcel(): void {
    ...
    this.inProgress = true;
    this.constantService.postRequest(this.constantService.REPORT_URI + 'book28/requestReportExcel/'
      , {
        'fromDate': this.datePipe.transform(this.searchModel.fromDate, 'dd/MM/yyyy'),
        'toDate': this.datePipe.transform(this.searchModel.toDate, 'dd/MM/yyyy'),
        'accuCode': this.searchModel.accuCode,
      ...
      .then(resJson => {
        if (resJson.responseCode === '0000') {
          ...
          const downloadLink = document.createElement('a');
          const fileName = 'Sổ_28_kiểm_sát_thi_hành_án_hình_phạt_bổ_sung_' + this.dateService.convertDateToStringByPattern(Date.now(), 'yyyyMMdd_HHmmss') + '.xlsx';
          downloadLink.href = linkSource;
          downloadLink.download = fileName;
          downloadLink.click();
        } else {
  ...
  exportDocx(): void {
    ...
    this.inProgress = true;
    this.constantService.postRequest(this.constantService.REPORT_URI + 'book28/requestReportDocx/'
      , {
        'fromDate': this.datePipe.transform(this.searchModel.fromDate, 'dd/MM/yyyy'),
        'toDate': this.datePipe.transform(this.searchModel.toDate, 'dd/MM/yyyy'),
        'accuCode': this.searchModel.accuCode,
      ...
      .then(resJson => {
        if (resJson.responseCode === '0000') {
          ...
          const downloadLink = document.createElement('a');
          const fileName = 'Sổ_28_kiểm_sát_thi_hành_án_hình_phạt_bổ_sung_' + this.dateService.convertDateToStringByPattern(Date.now(), 'yyyyMMdd_HHmmss') + '.docx';
          downloadLink.href = linkSource;
          downloadLink.download = fileName;
          downloadLink.click();
        } else {
  ...
  search(searchKeyword: string): void {
    const filteredList: Book28[] = [];
    let stt = 1;
    this.copyData().forEach((value: Book28) => {
