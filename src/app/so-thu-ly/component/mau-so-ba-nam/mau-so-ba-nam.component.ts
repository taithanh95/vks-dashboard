...
import {Book35, Book35Search, ColsTable} from '../../model/so-thu-ly.model';
...
@Component({
  selector: 'app-mau-so-ba-nam',
  templateUrl: './mau-so-ba-nam.component.html',
  providers: [DatePipe]
})
export class MauSoBaNamComponent implements OnInit {
  totalPages: number;
  totalRecords: number;
  ...
  getListData() {
    ...
    this.constantService.postRequest(this.constantService.REPORT_URI + 'book35/requestReport/'
      , {
  ...
  exportPdf(): void {
    ...
    this.inProgress = true;
    this.constantService.postRequest(this.constantService.REPORT_URI + 'book35/requestReportPdf/'
      , {
        'fromDate': this.datePipe.transform(this.searchModel.fromDate, 'dd/MM/yyyy'),
        'toDate': this.datePipe.transform(this.searchModel.toDate, 'dd/MM/yyyy'),
        'unitId': this.searchModel.unitId
      ...
      .then(resJson => {
        if (resJson.responseCode === '0000') {
          ...
          const fileName = 'SỔ_35_GIẢI_QUYẾT_TƯƠNG_TRỢ_TƯ_PHÁP_VỀ_HÌNH_SỰ_CỦA_VIỆT_NAM_' +
