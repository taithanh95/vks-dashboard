...
@Component({
  selector: 'app-create-request-report',
  templateUrl: './create-request-report.component.html',
  styleUrls: ['./create-request-report.component.css'],
  providers: [
    {
...
export class CreateRequestReportComponent implements OnInit {
  ...
  onSubmit(): void {
    ...
    if (this.formData.valid) {
      ...
      if (this.RPT_OLD.includes(code)) {
        ...
        // this.constantService.postRequest(this.constantService.REPORT_URI + 'requestReport/requestReportAPI', request).toPromise()
        //   .then(resp => resp.json())
        //   .then(resJson => {
        //     if (resJson.responseCode === ResponseCode.SUCCESS && resJson.responseData.statusCodeValue === 200) {
        //       console.log('resJson.responseData', resJson.responseData);
        //       // Đổi trạng thái từ đang xử lý => Thành công
        //       this.changeStatus(genId, 3);
        //     } else {
        //       // Ngược lại => Lỗi
        //       this.changeStatus(genId, 4);
        //     }
        //   }).catch(() => {
        //   this.toastrService.error('Hệ thống không có phản hồiiiiiiii.');
        //   this.isLoading = false;
        //   this.changeStatus(genId, 4);
        // });
        this.constantService.postRequest(this.constantService.REPORT_URI + 'requestReport/requestReportAPI', request).toPromise().then(resp => resp.json());
      }
  ...
  createRequestReport(request) {
    this.constantService.postRequest(this.constantService.REPORT_URI + 'requestReport/create/', request).toPromise()
      .then(resp => resp.json())
      .then(resJson => {
  ...
  changeStatus(genId: String, status: Number) {
    this.constantService.getRequest(this.constantService.REPORT_URI + `requestReport/changeStatus?genId=${genId}&status=${status}`)
      .toPromise()
      .then(resp => resp.json())
      .then(resJson => {
        if (resJson.responseCode === ResponseCode.SUCCESS) {
