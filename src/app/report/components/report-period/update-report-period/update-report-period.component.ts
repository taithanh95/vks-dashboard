...
@Component({
  selector: 'app-update-report-period',
  templateUrl: './update-report-period.component.html',
  styleUrls: ['./update-report-period.component.css'],
  providers: [
    {
...
export class UpdateReportPeriodComponent implements OnInit {
  ...
  onSubmit(): void {
    ...
    if (this.formData.valid) {
      ...
      this.constantService.postRequest(this.constantService.REPORT_URI + 'requestPeriodReport/updateReportInput/', request).toPromise()
        .then(resp => resp.json())
        .then(resJson => {
